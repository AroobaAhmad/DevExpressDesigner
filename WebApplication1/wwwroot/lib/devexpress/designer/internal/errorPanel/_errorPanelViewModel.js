...
export class ErrorPanelViewModel extends Disposable {
    ...
    getNotificationTemplate() {
        const filteredErrorList = this._errorList().filter(x => this._suppressedErrorCodes().indexOf(x.code) === -1);
        return filteredErrorList.length > 0 ? 'dxrd-svg-errorPanel-notification' : 'dxrd-svg-errorPanel-notification_empty';
    }
    getTitleMessage() {
